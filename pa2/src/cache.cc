...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        if (cache_type == IS_ITLB || cache_type == IS_DTLB || cache_type == IS_STLB)
        {
            ...
            if (MSHR.entry[mshr_index].data == (UINT64_MAX >> LOG2_PAGE_SIZE))
            {
                ...
                if (MSHR.entry[mshr_index].type == TRANSLATION_FROM_L1D || MSHR.entry[mshr_index].type == PREFETCH_TRANSLATION)
                {
                    ...
                    MSHR.num_returned--;
                    update_fill_cycle();
                    // cout << "exitng fshaldnle fill" << endl;
                    return;
                }
        ...
        if (do_fill)
        {
            ...
            if ((cache_type == IS_ITLB) && (MSHR.entry[mshr_index].type != PREFETCH_TRANSLATION))
            { //@v Responses to prefetch requests should not go to PROCESSED queue
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
        {
            ...
            { // read hit
                ...
                if (RQ.entry[index].type == LOAD)
                {
                    ...
                    { // Neelu: for dense region, only invoking on loads, check other l2c_pref_operate as well.
...
void CACHE::handle_prefetch()
{
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
        {
            ...
            {
                pair<uint32_t, uint32_t> p = check_hit_randomized(&PQ.entry[index]);
                set = p.first;
                way = p.second;
            }
...
pair<uint32_t, uint32_t> CACHE::check_hit_randomized(PACKET *packet)
{
    ...
    bool found = 0;
    // cout << "checking " << endl;
    for (curr_way = 0; curr_way < dynamic_set.size(); ++curr_way)
    {
        curr_set = dynamic_set[curr_way] % NUM_SET;
        // cout << "way: " << curr_way << ", set: " << curr_set << endl;
        if (block[curr_set][curr_way].ttl > 0 && (block[curr_set][curr_way].tag == packet->address))
        {
            // cout << "found smthg" << endl;
            DP(if (warmup_complete[packet->cpu]) {
                            cout << "[" << NAME << "] " << __func__ << " instr_id: " << packet->instr_id << " type: " << +packet->type << hex << " addr: " << packet->address;
                            cout << " full_addr: " << packet->full_addr << " tag: " << block[curr_set][curr_way].tag << " data: " << block[curr_set][curr_way].data << dec;
                            cout << " set: " << curr_set << " way: " << curr_way << " lru: " << block[curr_set][curr_way].lru;
                            cout << " event: " << packet->event_cycle << " cycle: " << current_core_cycle[cpu] << endl; });
            found = 1;
            break;
        }
...
#endif
    // cout << "exit check_hit randomized" << endl;
    if (!found)
        return {-1, -1};
    return {curr_set, curr_way};
}
...
{
    // cout << "prefetch_translation" << endl;
    pf_requested++;
...
{
    // cout << "increment_WQ_FULL" << endl;
    WQ.FULL++;
}

uint64_t CACHE::total_cycles()
{
    // cout << "total_cycles" << endl;
    uint64_t res = 0;
    for (int cpu = 0; cpu < NUM_CPUS; cpu++)
    {
        res += current_core_cycle[cpu];
    }
}

void CACHE::update_rttl(bool conflict)
{
    // cout << "update_rttl" << endl;
    int curr_total_cycles = total_cycles();
    if (curr_total_cycles - last_rttl_update > rttl_update_threshold)
    {
        last_rttl_update = curr_total_cycles;
        if (conflict)
            rTTL *= 2;
        else
            rTTL -= 1;
