...
void PAGE_TABLE_WALKER::operate()
{
            ...
            if (MSHR.entry[index].translation_level == 0 && !dropped_prefetch_request) // If translation complete
            {
                ...
                for (int i = 5; i > 1; i--) // Walk the page table and fill MMU caches
                {
                    ...
                    if (MSHR.entry[index].init_translation_level - i >= 0) // Check which translation levels needs to filled
                    {
                        ...
                        {
                        case 5:
                            fill_mmu_cache(PSCL5, next_level_base_addr, &MSHR.entry[index], IS_PSCL5);
                            break;
                        case 4:
                            fill_mmu_cache(PSCL4, next_level_base_addr, &MSHR.entry[index], IS_PSCL4);
                            break;
                        case 3:
                            fill_mmu_cache(PSCL3, next_level_base_addr, &MSHR.entry[index], IS_PSCL3);
                            break;
                        case 2:
                            fill_mmu_cache(PSCL2, next_level_base_addr, &MSHR.entry[index], IS_PSCL2);
                            break;
                        }
                    }
                }
    ...
    else if (RQ.occupancy > 0) // If there is no pending request which is undergoing translation, then process new request.
    {
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[cpu]) && (ooo_cpu[cpu].L2C.RQ.occupancy < ooo_cpu[cpu].L2C.RQ.SIZE))
        {
            ...
            uint64_t address_pscl5 = check_hit(PSCL5, get_index(RQ.entry[index].full_addr, IS_PSCL5), RQ.entry[index].type, RQ.entry[index].full_addr);
            uint64_t address_pscl4 = check_hit(PSCL4, get_index(RQ.entry[index].full_addr, IS_PSCL4), RQ.entry[index].type, RQ.entry[index].full_addr);
            uint64_t address_pscl3 = check_hit(PSCL3, get_index(RQ.entry[index].full_addr, IS_PSCL3), RQ.entry[index].type, RQ.entry[index].full_addr);
            uint64_t address_pscl2 = check_hit(PSCL2, get_index(RQ.entry[index].full_addr, IS_PSCL2), RQ.entry[index].type, RQ.entry[index].full_addr);
            ...
            else
            {
                if (CR3_addr == UINT64_MAX)
                {
                    ...
                    for (int i = 5; i > 1; i--) // Fill MMU caches
                    {
                        ...
                        {
                        case 5:
                            fill_mmu_cache(PSCL5, next_level_base_addr, &RQ.entry[index], IS_PSCL5);
                            break;
                        case 4:
                            fill_mmu_cache(PSCL4, next_level_base_addr, &RQ.entry[index], IS_PSCL4);
                            break;
                        case 3:
                            fill_mmu_cache(PSCL3, next_level_base_addr, &RQ.entry[index], IS_PSCL3);
                            break;
                        case 2:
                            fill_mmu_cache(PSCL2, next_level_base_addr, &RQ.entry[index], IS_PSCL2);
                            break;
                        }
                    }
    ...
    else if (PQ.occupancy > 0)
    {
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[cpu]) && (ooo_cpu[cpu].L2C.RQ.occupancy < ooo_cpu[cpu].L2C.RQ.SIZE))
        {
            ...
            uint64_t address_pscl5 = check_hit(PSCL5, get_index(PQ.entry[index].full_addr, IS_PSCL5), PQ.entry[index].type, PQ.entry[index].full_addr);
            uint64_t address_pscl4 = check_hit(PSCL4, get_index(PQ.entry[index].full_addr, IS_PSCL4), PQ.entry[index].type, PQ.entry[index].full_addr);
            uint64_t address_pscl3 = check_hit(PSCL3, get_index(PQ.entry[index].full_addr, IS_PSCL3), PQ.entry[index].type, PQ.entry[index].full_addr);
            uint64_t address_pscl2 = check_hit(PSCL2, get_index(PQ.entry[index].full_addr, IS_PSCL2), PQ.entry[index].type, PQ.entry[index].full_addr);
            ...
            else
            {
                if (CR3_addr == UINT64_MAX)
                {
                    ...
                    for (int i = 5; i > 1; i--) // Fill MMU caches
                    {
                        ...
                        {
                        case 5:
                            fill_mmu_cache(PSCL5, next_level_base_addr, &PQ.entry[index], IS_PSCL5);
                            break;
                        case 4:
                            fill_mmu_cache(PSCL4, next_level_base_addr, &PQ.entry[index], IS_PSCL4);
                            break;
                        case 3:
                            fill_mmu_cache(PSCL3, next_level_base_addr, &PQ.entry[index], IS_PSCL3);
                            break;
                        case 2:
                            fill_mmu_cache(PSCL2, next_level_base_addr, &PQ.entry[index], IS_PSCL2);
                            break;
                        }
                    }
    ...
    { // no VA => PA translation found
        ...
        else
        {
            ...
            {                                                   // try to find an empty physical page number
                ...
                if (ppage_check != inverse_table.end() || random_ppage == (UINT64_MAX >> LOG2_PAGE_SIZE))
                { // random_ppage is not available //@Vishal: Don't allocale all FFFF page
...
uint64_t PAGE_TABLE_WALKER::check_hit(CACHE &cache, uint64_t address, uint8_t type, uint64_t full_addr)
{
    //DEBUG_END();
    if (cache.cache_type != IS_LLC)
    {
        ...
        for (uint32_t way = 0; way < cache.NUM_WAY; way++)
        {
            if (cache.block[set][way].valid && (cache.block[set][way].tag == address))
            {
                ...
                // cout << "proper value returned" << endl;
                return cache.block[set][way].data;
            }
        }

        // cout << "rubbish returned" << endl;
        //DEBUG_END();
        return UINT64_MAX;
    }
    else
    {
        // cout << "evil 5" << endl;
        // uint32_t set = cache.get_set(address);
        vector<uint32_t> dynamic_set = cache.get_dynamic_set(full_addr);
        ...
        for (uint32_t way = 0; way < dynamic_set.size(); ++way)
        {
            uint32_t set = dynamic_set[way] % cache.NUM_SET;
            if (cache.block[set][way].valid && (cache.block[set][way].tag == address))
            {
