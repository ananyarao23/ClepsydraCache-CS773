...
public:
    ...
    uint8_t cache_type;
    uint32_t lTTL, uTTL, rTTL, lRate, uRate;
    // lTTL and uTTL are bounds for uniform sampling for each cache block, lRate and uRate are the bounds we maintain while updating rTTL
    // prefetch stats
        ...
        : NAME(v1), NUM_SET(v2), NUM_WAY(v3), NUM_LINE(v4), WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8)
    {
        ...
        find_randomized_victim = &CACHE::ttl_victim;
    ...
    void add_nonfifo_queue(PACKET_QUEUE *queue, PACKET *packet), //@Vishal: Updated from add_mshr
        ...
        ttl_update(uint32_t set, uint32_t way, uint64_t full_addr),
    ...
    uint32_t get_set(uint64_t address),
        ...
        (CACHE::*find_randomized_victim)(uint32_t cpu, uint64_t instr_id, uint32_t &set, uint64_t ip, uint64_t full_addr, uint32_t type),
